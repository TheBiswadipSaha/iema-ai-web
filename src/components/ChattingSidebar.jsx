...
export const ChattingSidebar = ({ pageConfig, onFilterChange }) => {
  ...
  return (
    <div className="w-80 bg-[#0A0A0B] border-r border-gray-800/50 h-screen overflow-y-auto flex flex-col">
      {/* Header */}
      ...
      {activeTab === 'generate' && (
        <div className="px-6 py-4 border-b border-gray-800/50 flex items-center justify-between">
          ...
          {activeCount > 0 && (
            <button onClick={clearAll} className="text-gray-400 cursor-pointer hover:text-white text-xs flex items-center gap-1">
              <X size={14} />
              Clear
            </button>
          )}
            ...
            return (
              <div key={idx} className="rounded-xl border border-gray-800/50 bg-[#0F0F11] overflow-hidden hover:border-gray-700/50">
                ...
                >
                  ...
                  <ChevronDown size={18} className={`text-gray-400 transition-transform pointer-events-none ${isExpanded ? 'rotate-180' : ''}`} />
                </button>
              ...
              {conversations.map((conversation, idx) => (
                ...
                >
                  <div className="flex items-start justify-between mb-2">
                    <h4 className="text-white text-sm font-medium line-clamp-1 flex-1 group-hover:text-emerald-400 transition-colors">
        ...
        <div className="rounded-xl border border-gray-800/50 bg-[#0F0F11] p-4 hover:border-gray-700/50 transition-all">
          ...
          <div className="flex items-center gap-1 mb-4">
            <div className="flex-1 h-1.5 bg-gray-800 rounded-full overflow-hidden">
              <div className="h-full bg-gradient-to-r from-emerald-500 to-emerald-400 w-[65%] rounded-full"></div>
            </div>
          </div>
